...
from backend.monolith.models.models import Song, SongListSaveModel
...
async def get_songlist(
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_200_OK, content={"songs": songs_data}
        )
...
async def add_song_to_songlist(
    ...
    try:
        ...
        if team_name:
            ...
            if not can_write:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail=write_msg
                )

        # Validate song access
        can_access, access_msg = validate_song_access_for_list(
            db, user_id, song_id, team_name
        )
        if not can_access:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=access_msg
            )
        ...
        success, message = add_song_to_list(
            db, user_id, songlist, song_id, song_title, team_name
        )
...
def save_song_list_with_titles(
    ...
    try:
        ...
        if success:
            return (
                True,
                f"Song list {songlist_id} saved with {len(song_data_list)} songs",
            )
        else:
