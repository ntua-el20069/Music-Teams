...
from sqlalchemy import exc as sqlalchemy_exc, distinct
...
def get_all_public_songs(db: Session) -> Tuple[bool, str, List[str]]:
    ...
    """
    try:
        songs = db.query(Song.title).filter(Song.public == True).all()
...
def get_all_composers_in_user_teams(db: Session, user_id: str) -> Tuple[bool, str, List[str]]:
    """
...
def get_all_lyricists_in_user_teams(db: Session, user_id: str) -> Tuple[bool, str, List[str]]:
    """
...
def get_all_songs_in_user_teams(db: Session, user_id: str) -> Tuple[bool, str, List[str]]:
    """
...
def get_all_composers_in_team(db: Session, team_name: str) -> Tuple[bool, str, List[str]]:
    """
    ...
    try:
        ...
        composer_names = [composer[0] for composer in composers]
        return True, f"Composers in team {team_name} retrieved successfully", composer_names
...
def get_all_lyricists_in_team(db: Session, team_name: str) -> Tuple[bool, str, List[str]]:
    """
    ...
    try:
        ...
        lyricist_names = [lyricist[0] for lyricist in lyricists]
        return True, f"Lyricists in team {team_name} retrieved successfully", lyricist_names
